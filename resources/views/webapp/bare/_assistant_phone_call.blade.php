<div class="container-fluid">
    <div class="card mb-4">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">Start Phone Call</h5>
        </div>
        <div class="card-body">
            <form id="assistantPhoneForm">
                <div class="mb-3">
...
<script>
// Assistant Phone Call management
const AssistantPhoneCall = {
    init() {
        this.loadAssistants();
        this.loadContacts();
        this.loadPipelines();
    ...
    attachEventListeners() {
        document.getElementById('assistantPhoneForm').addEventListener('submit', (e) => {
            e.preventDefault();
            this.startCall();
        });
    ...
    async startCall() {
        ...
        document.getElementById('assistantPhoneForm').closest('.card').style.display = 'none';
        ...
        try {
            ...
            const response = await fetch('/api/assistant/call', {
                ...
                body: JSON.stringify({
                    ...
                    conversation_path_id: conversationPathId || null,
    ...
    async endCall() {
        ...
        try {
            const response = await fetch(`/api/assistant/call/${this.currentCallId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${appState.apiToken}`
                }
    ...
    startStatusPolling() {
        ...
        this.statusInterval = setInterval(async () => {
            ...
            try {
                const response = await fetch(`/api/assistant/call/${this.currentCallId}/status`, {
                    headers: {
                        'Authorization': `Bearer ${appState.apiToken}`
                    }
                });
    ...
    resetInterface() {
        ...
        // Reset UI
        document.getElementById('assistantPhoneForm').closest('.card').style.display = 'block';
        document.getElementById('callStatus').style.display = 'none';
        document.getElementById('endCallButton').style.display = 'none';
        document.getElementById('assistantPhoneForm').reset();
    }
};

// Initialize when document is ready
AssistantPhoneCall.init();
</script> 