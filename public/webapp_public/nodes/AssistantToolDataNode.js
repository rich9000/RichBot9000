class AssistantToolDataNode extends DataNode {
    constructor(data = {}) {
        ...
        this.description = 'Execute a tool and store results in path state';
        this.toolHandler = new AssistantToolHandler(this);
    }

    getNodeInfo(context = {}) {
        console.log('[AssistantToolDataNode] getNodeInfo context:', context);
        ...
        console.log('[AssistantToolDataNode] Available tools:', tools);

        return this.content.tools.map(tool => {
            const toolInfo = tools.find(t => t.id === tool.toolId);
            console.log('[AssistantToolDataNode] Tool info for', tool.toolId, ':', toolInfo);
            return toolInfo ? `${toolInfo.name} → ${tool.pathStateKey || 'No path state key'}` : 'Unknown Tool';
        }).join(', ');
    }
    ...
    getDetailsHtml(nodeIndex, context = {}) {
        ...
        if (!this.content.tools || !this.content.tools.length) {
            ...
            html = this.content.tools.map(tool => {
                ...
                // Add path state key
                toolHtml += `<br>Path State Key: ${tool.pathStateKey || 'Not set'}`;
    ...
    updateToolPathStateKey(toolIndex, value) {
        if (this.content.tools && this.content.tools[toolIndex]) {
            this.content.tools[toolIndex].pathStateKey = value;
        }
    }
    ...
    getNodeCardHtml(nodeIndex, context = {}) {
        return `
            <div class="conversation-node card mb-3 data assistantTool" data-node-index="${nodeIndex}" style="max-width:540px;margin-left:auto;margin-right:auto;">
...
if (typeof window !== 'undefined') {
    window.AssistantToolDataNode = AssistantToolDataNode;
} 