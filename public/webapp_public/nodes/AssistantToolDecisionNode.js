class AssistantToolDecisionNode extends DecisionNode {
    constructor(data = {}) {
        ...
        this.description = 'Execute a tool and branch based on result';
        this.toolHandler = new AssistantToolHandler(this);
    }

    getNodeInfo(context = {}) {
        console.log('[AssistantToolDecisionNode] getNodeInfo context:', context);
        ...
        console.log('[AssistantToolDecisionNode] Available tools:', tools);

        return this.content.tools.map(tool => {
            const toolInfo = tools.find(t => t.id === tool.toolId);
            console.log('[AssistantToolDecisionNode] Tool info for', tool.toolId, ':', toolInfo);
    ...
    getNodeCardHtml(nodeIndex, context = {}) {
        return `
            <div class="conversation-node card mb-3 decision assistantTool" data-node-index="${nodeIndex}" style="max-width:540px;margin-left:auto;margin-right:auto;">
    ...
    updateToolCondition(toolIndex, value) {
        this.toolHandler.updateToolCondition(toolIndex, value);
    }
...
if (typeof window !== 'undefined') {
    window.AssistantToolDecisionNode = AssistantToolDecisionNode;
} 